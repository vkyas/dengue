...
# =================================================================
# KONFIGURASI TARGET: PHP 8.4.8 dengan modul shared
# =================================================================
export NDK_VERSION="r26d"
export PHP_VERSION="8.4.8"
export API_LEVEL="22"
...
# ---- Pembersihan Paksa untuk memastikan build yang bersih ----
step "Membersihkan lingkungan build sebelumnya"
rm -f "$LOG_FILE"
rm -rf "$WORKDIR" "$OUTDIR"
...
if [ ! -f "$PREFIX/lib/libssl.a" ]; then
  ...
  [ ! -f "$OPENSSL_FILE" ] && wget -q https://www.openssl.org/source/${OPENSSL_FILE}
  tar -xzf "$OPENSSL_FILE" && cd "openssl-${OPENSSL_VERSION}"
  # PERBAIKAN: Menggunakan konfigurasi yang lebih optimal untuk ARM
...
step "Build ICU4C (statis)"
if [ ! -f "$PREFIX/lib/libicuuc.a" ]; then
    ICU_VERSION="75.1"; ICU_VERSION_US="75_1"; ICU_SRC_DIR="icu"; ICU_FILE="icu4c-${ICU_VERSION_US}-src.tgz"; ICU_HOST_BUILD_DIR="$WORKDIR/icu-host-build"; ICU_TARGET_BUILD_DIR="$WORKDIR/icu-target-build"
    [ ! -f "$ICU_FILE" ] && wget -q https://github.com/unicode-org/icu/releases/download/release-${ICU_VERSION//./-}/${ICU_FILE}; [ ! -d "$ICU_SRC_DIR" ] && tar -xzf "$ICU_FILE"
    step "Build ICU4C - Tahap 1: Host Tools"; mkdir -p "$ICU_HOST_BUILD_DIR" && cd "$ICU_HOST_BUILD_DIR"
    CC="gcc" CXX="g++" CFLAGS="" LDFLAGS="" CPPFLAGS="" ../$ICU_SRC_DIR/source/configure --prefix="$ICU_HOST_BUILD_DIR/install" --disable-samples --disable-tests; CC="gcc" CXX="g++" make -j$(nproc); cd "$WORKDIR"
    step "Build ICU4C - Tahap 2: Target Library"; mkdir -p "$ICU_TARGET_BUILD_DIR" && cd "$ICU_TARGET_BUILD_DIR"
    ../$ICU_SRC_DIR/source/configure --host="$HOST_TRIPLE" --prefix="$PREFIX" --with-cross-build="$ICU_HOST_BUILD_DIR" --disable-shared --enable-static --with-data-packaging=static --disable-samples --disable-tests
    make -j$(nproc) && make install; cd "$WORKDIR"; fi
...
step "Build libcurl (statis)"
if [ ! -f "$PREFIX/lib/libcurl.a" ]; then
  CURL_VERSION="8.7.1"; CURL_FILE="curl-${CURL_VERSION}.tar.xz"; [ ! -f "$CURL_FILE" ] && wget -q https://curl.se/download/${CURL_FILE}; tar -xf "$CURL_FILE" && cd "curl-${CURL_VERSION}"; ./configure --host="$HOST_TRIPLE" --prefix="$PREFIX" --disable-shared --enable-static --with-ssl="$PREFIX" --without-zstd --disable-ldap; make -j$(nproc) && make install; cd "$WORKDIR"; fi

# =================================================================
# BUILD PHP 8.4.8
# =================================================================
step "Unduh dan konfigurasikan PHP"
PHP_SRC="php-$PHP_VERSION"; PHP_FILE="php-${PHP_VERSION}.tar.gz"; [ ! -d "$PHP_SRC" ] && { [ ! -f "$PHP_FILE" ] && wget -q https://github.com/vkyas/dengue/releases/download/php-8.4.8/php-${PHP_VERSION}.tar.gz; tar -xzf "$PHP_FILE"; }; cd "$PHP_SRC"; [ -f "Makefile" ] && make distclean; ./buildconf --force;

# PERBAIKAN: Menghapus flag --enable-wddx dan --enable-xmlrpc yang tidak valid untuk PHP 8.4
# PERBAIKAN: Menyederhanakan LIBS, biarkan --with-xx-dir yang bekerja
./configure \
  --build=x86_64-pc-linux-gnu \
  --host="$HOST_TRIPLE" \
  --prefix="$OUTDIR" \
  --with-config-file-path="$OUTDIR/etc" \
  --with-config-file-scan-dir="$OUTDIR/etc/conf.d" \
  --enable-shared \
  --disable-static \
  --with-zlib \
  --enable-ctype \
...
# =================================================================
# BUILD EKSTENSI PECL (OPSIONAL)
# =================================================================
PHP_EXTENSION_DIR=$(find "$OUTDIR/lib/php/extensions" -type d -name "no-debug-non-zts-*")
mkdir -p "$PHP_EXTENSION_DIR"

build_pecl_extension() {
  EXT_NAME="$1"; EXT_VERSION="$2"; EXT_URL="$3"
  step "Build $EXT_NAME-$EXT_VERSION"
  if [ ! -f "$PHP_EXTENSION_DIR/$EXT_NAME.so" ]; then
    EXT_FILE="${EXT_NAME}-${EXT_VERSION}.tgz"; [ ! -f "$EXT_FILE" ] && wget -q "$EXT_URL"
    tar -xzf "$EXT_FILE" && cd "${EXT_NAME}-${EXT_VERSION}"
    "$OUTDIR/bin/phpize"; ./configure --with-php-config="$OUTDIR/bin/php-config"; make -j$(nproc) && make install
    cd "$WORKDIR"
  fi
}
...
log_errors = Off
enable_dl = On
extension_dir = "/data/adb/php8/files/bin"
cgi.fix_pathinfo=1
EOF

# PERBAIKAN: Menghapus ekstensi yang tidak bisa di-build dari daftar
cat > "$INI_DIR/conf.d/modules.ini" << 'EOF'
extension=ctype
extension=curl
extension=dom
extension=filter
